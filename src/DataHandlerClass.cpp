...
void *DataUARTHandler::sortIncomingData( void )
{
    ...
    boost::shared_ptr<pcl::PointCloud<PointRADAR>> RScan(new pcl::PointCloud<PointRADAR>);
        ...
        case READ_OBJ_STRUCT:
            ...
            while( i < mmwData.numObjOut ) {
                if (((mmwData.header.version >> 24) & 0xFF) < 3) { // SDK version is older than 3.x
                    ...
                    RScan->points[i].az = std::atan2(-mmwData.newObjOut.x,mmwData.newObjOut.y) / M_PI * 180;
                    RScan->points[i].vr = mmwData.newObjOut.velocity;
                    ...
                    radarscan.bearing = std::atan2(-mmwData.newObjOut.x,mmwData.newObjOut.y) / M_PI * 180;
